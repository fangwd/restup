...
MyDB.prototype.dump = function() {
  let db = {}
  for (let name in this._tableMap) {
    let table = this._tableMap[name]
    db[name] = table._recordList.map(record => {
      let row = {}
      for (let key in record) {
        if (!/^__/.exec(key) && typeof record[key] !== 'function') {
          if (record[key] instanceof Record) {
            let parent = record[key], tableName = parent.__table.name
            row[key] = `Record(${record[key].__table.name})`
          }
          else {
            row[key] = record[key]
          }
