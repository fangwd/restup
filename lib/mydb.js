...
Record.prototype.__json = function(filterType) {
  ...
  for (let key in this) {
    ...
    if (/^__/.exec(key)) {
      ...
      row[key] = `Record(${this[key].__table.name})`
    }
...
MyDB.prototype.dump = function(filterType=0) {
  let db = {}
  for (let name in this._tableMap) {
    let table = this._tableMap[name]
    db[name] = table._recordList.map(rec => rec.__json(filterType))
  }
  return db
}
