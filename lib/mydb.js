...
function Database(data) {
  ...
  this._data.tables.forEach(table => {
    this._tableMap[table.name] = new Table(this, table)
  })
}
      ...
      return keys.map(key => {
        if (key.indexOf(null) >= 0) {
          ...
          for (let i = 0; i < names.length; i++) {
            if (key[i] === null) {
              parts.push(names[i] + ' IS NULL')
            }
            else {
              parts.push(names[i] + '=' + MySQL.escape(key[i]))
            }
          }
...
function Record(table) {
  ...
  this.__primaryKey = function() {
    assert(table.primaryKey.length === 1, 'Bad key: ' + table.name)
...
Record.prototype.__json = function(filterType) {
  let row = {}
  for (let key in this) {
    let field = this[key]
    if (/^__/.exec(key)) {
      ...
      row[key] = `Record(${this[key].__table.name})` + JSON.stringify(this[key].__json())
    }
...
Table.prototype.__json = function(filterType) {
  return this._recordList.map(rec => rec.__json(filterType))
}

MyDB.prototype.__json = function(filterType=0) {
  let db = {}
  for (let name in this._tableMap) {
    db[name] = this._tableMap[name].__json()
  }
  return db
}
...
Table.prototype.flush = function() {
    ...
    if (flushable) {
      ...
      for (let key in keys) {
        ...
        if (typeof row[key] === 'string' && (
            this.uniqueConstraint.indexOf(key) >= 0 ||
            this.primaryKey.indexOf(key) >= 0)) {
  ...
  if (rows.length == 0) {
    if (!this.__flush.dirty) {
      console.log(JSON.stringify(this.__json(), null, 4))
      assert(0, "__flush.dirty cleared: " + this.name)
    }
...
MyDB.prototype.flush = function(clear=true) {
  const tables = Object.keys(this._tableMap).map(key => this._tableMap[key])

  for (let i = 0; i < tables.length; i++) {
    let dirty = tables[i]._recordList.length > 0
    tables[i].__flush = { counter: 0, dirty: dirty }
  }
      ...
      if (table) {
        ...
        if (dirty_tables > 0) {
          if (flushed_rows > 0) {
            ...
            let names = tables.filter(table => table.__flush.dirty)
                  .map(table => table.name).join(', ')
            tables.filter(table => table.__flush.dirty).map(table => {
              console.log('Table: ' + table.name)
              console.log(JSON.stringify(table.__json(), null, 4))
            })
            reject(Error(`Can't flush incomplete/cyclic row(s) in ${names}`))
          }
        }
        else {
          if (clear) self.clear()
...
MyDB.prototype.clear = function() {
  for (let name in this._tableMap) {
    this._tableMap[name]._recordList = []
  }
}
